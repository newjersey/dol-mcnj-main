...
fs.readFile(path.join(__dirname, csvFilename), function (err, fileContents) {
  ...
  // Try to identify a unique key column (common names)
  const possibleKeyColumns = ['program_id', 'id', 'key', 'program_key'];
  const keyColumn = possibleKeyColumns.find(col => columns.includes(col));
  ...
  data.forEach((row, idx) => {
    ...
    // Check for duplicates if we found a key column
    if (keyColumn && row[keyColumn]) {
      const keyValue = typeof row[keyColumn] === "string" ? row[keyColumn].trim() : row[keyColumn];
      
      // Skip rows with null/empty key values
      if (nullValues.has(keyValue)) {
        skippedRows.push({ idx, reason: `missing required ${keyColumn}` });
        return;
      }
      
      // Check for duplicates
      if (seenKeys.has(keyValue)) {
        skippedRows.push({ idx, reason: `duplicate ${keyColumn}: ${keyValue}` });
        return;
      }
