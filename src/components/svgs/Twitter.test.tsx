...
describe("Twitter Component", () => {
  it("renders without crashing with default props", () => {
    ...
    expect(svg).toHaveClass(
      "svg-inline--fa fa-twitter-square fa-w-14 fa-2x nj-gray",
    );
    expect(svg).toHaveAttribute("aria-hidden", "true");
    expect(svg).toHaveAttribute("focusable", "false");
    expect(svg).toHaveAttribute("data-prefix", "fab");
    expect(svg).toHaveAttribute("data-icon", "twitter-square");
  ...
  it("renders with custom color prop", () => {
    ...
    const { container } = render(<Twitter color={customColor} />);
    expect(container).toBeInTheDocument();
    const svg = container.querySelector("svg") as SVGSVGElement;
    const path = svg.querySelector("path");
