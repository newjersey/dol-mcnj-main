...
export const getZipCodesInRadius = (
  zipCode: string,
...
// import { zipCodes } from "./zipCodeCoordinates";

// export function getZipCodesInRadius(center: string, radius: number): string[] {
//   // Convert center and zip codes to numbers
//   const centerNum = parseInt(center, 10);
//   const zipCodesNum = zipCodes.map((zip) => parseInt(zip, 10));

//   // Filter zip codes within the radius
//   const zipCodesInRadius = zipCodesNum.filter(
//     (zip) => Math.abs(zip - centerNum) <= radius * 40,
//   );

//   // Convert back to strings
//   return zipCodesInRadius.map((zip) => `0${zip.toString()}`);
// }
