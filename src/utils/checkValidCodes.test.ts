...
describe("checkValidSocCode", () => {
  ...
  it("returns formatted SOC code if it has six digits and a valid prefix", () => {
    const value = "111011";
    const result = checkValidSocCode(value);
    expect(result).toEqual("11-1011");
  });

  it("returns the value if it has six digits but invalid SOC prefix", () => {
    const value = "123456";
    const result = checkValidSocCode(value);
    expect(result).toEqual(value);
  });
