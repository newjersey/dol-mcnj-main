export const FOOTER_NAV_1_DATA = {
  title: "Footer Nav Menu_v2",
  heading: "My Career NJ",
  itemId: "6QDRPQOaswzG5gHPgqoOkS",
  en: {
    topLevelItemsCollection: {
      items: [
        {
          copy: "Find a Job",
          classes: "no-link",
          itemId: "671OHO6toaAKS1jIkyQYlS",
          subItemsCollection: {
            items: [
              {
                copy: "NJ Career Navigator",
                url: "/navigator",
                itemId: "7bUSUzaugCaaJiO8q5VFP2",
              },
              {
                copy: "In-Demand Occupations",
                url: "/in-demand-occupations",
                itemId: "1I96MEcDkMtqHj36y6O2Tu",
              },
              {
                copy: "One Stop Job Board",
                url: "https://www.careeronestop.org/",
                itemId: "6qnHxliOmFomI5qoV1IdSz",
              },
              {
                copy: "Labor Market Information",
                url: "https://www.nj.gov/labor/labormarketinformation/",
                itemId: "4NWWyuPPrrwVV5OFcMOOmx",
              },
              {
                copy: "Apprenticeship Programs",
                url: "https://www.nj.gov/labor/career-services/apprenticeship/",
                itemId: "sPaifA4vlJxjk4S69hqjJ",
              },
            ],
          },
        },
        {
          copy: "Get Training",
          classes: "no-link",
          itemId: "2Q7fHTof7rUdSUdrzp2SDk",
          subItemsCollection: {
            items: [
              {
                copy: "NJ Training Explorer",
                url: "/training",
                itemId: "2TyVv5heGCi7c3GLNo7OIS",
              },
              {
                copy: "NJ Career Navigator",
                url: "/navigator",
                itemId: "7bUSUzaugCaaJiO8q5VFP2",
              },
              {
                copy: "Tuition Assistance",
                url: "/support-resources/tuition-assistance",
                itemId: "2gqhjZAj8srnVuvuVdXUBf",
              },
              {
                copy: "Training Provider Resources",
                url: "/training-provider-resources",
                itemId: "4Rrw3ktcpB7VQU1LcOpeTS",
              },
              {
                copy: "SkillUp NJ",
                url: "https://nj.metrixlearning.com/landing.cfm",
                itemId: "4bKG1PzbEV649oyXknBUUe",
              },
            ],
          },
        },
        {
          copy: "Explore Careers",
          itemId: "2ZTghf6TUoZgAKd2cUHy4P",
          subItemsCollection: {
            items: [
              {
                copy: "NJ Career Navigator",
                url: "/navigator",
                itemId: "7bUSUzaugCaaJiO8q5VFP2",
              },
              {
                copy: "NJ Career Pathways",
                url: "/career-pathways",
                itemId: "4qo1suhSHJek5pbDGLYqUZ",
              },
            ],
          },
        },
        {
          copy: "Find Support and Assistance",
          classes: "no-link",
          itemId: "5SPeK1gJLpMBLGuTFEAVmg",
          subItemsCollection: {
            items: [
              {
                copy: "Helpful Resources",
                url: "/support-resources",
                itemId: "4ec33mgyIj1ghaZ9GYNCGR",
              },
              {
                copy: "Training Provider Resources",
                url: "/training-provider-resources",
                itemId: "4eb9EtRlHD03bDlUfJ8mkt",
              },
              {
                copy: "Career Services",
                url: "https://nj.gov/labor/career-services/",
                itemId: "25V7xd5vZEG9BLihkBR7xW",
              },
            ],
          },
        },
        {
          copy: "Contact Us",
          classes: "no-link",
          itemId: "5rEIg0E1yASZgJXNm537PX",
          subItemsCollection: {
            items: [
              {
                copy: "Contact Us",
                url: "?contactModal=true",
                itemId: "2Mi5UQIdSBuhJXl1CckrWA",
              },
              {
                copy: "Press and Media",
                url: "/press-and-media",
                itemId: "3p1jv1b0gk3J5jYk2t1e8",
              },
            ],
          },
        },
      ],
    },
  },
  es: {
    topLevelItemsCollection: {
      items: [
        {
          copy: "Buscar Empleo",
          classes: "no-link",
          itemId: "671OHO6toaAKS1jIkyQYlS",
          subItemsCollection: {
            items: [
              {
                copy: "Navegador de Carreras de NJ",
                url: "/navigator",
                itemId: "7bUSUzaugCaaJiO8q5VFP2",
              },
              {
                copy: "Ocupaciones en Demanda",
                url: "/in-demand-occupations",
                itemId: "1I96MEcDkMtqHj36y6O2Tu",
              },
              {
                copy: "Bolsa de Trabajo One Stop",
                url: "https://www.careeronestop.org/",
                itemId: "6qnHxliOmFomI5qoV1IdSz",
              },
              {
                copy: "Información del Mercado Laboral",
                url: "https://www.nj.gov/labor/labormarketinformation/",
                itemId: "4NWWyuPPrrwVV5OFcMOOmx",
              },
              {
                copy: "Programas de Aprendizaje",
                url: "https://www.nj.gov/labor/career-services/apprenticeship/",
                itemId: "sPaifA4vlJxjk4S69hqjJ",
              },
            ],
          },
        },
        {
          copy: "Obtener Capacitación",
          classes: "no-link",
          itemId: "2Q7fHTof7rUdSUdrzp2SDk",
          subItemsCollection: {
            items: [
              {
                copy: "Explorador de Capacitación de NJ",
                url: "/training",
                itemId: "2TyVv5heGCi7c3GLNo7OIS",
              },
              {
                copy: "Navegador de Carreras de NJ",
                url: "/navigator",
                itemId: "7bUSUzaugCaaJiO8q5VFP2",
              },
              {
                copy: "Asistencia para Matrícula",
                url: "/support-resources/tuition-assistance",
                itemId: "2gqhjZAj8srnVuvuVdXUBf",
              },
              {
                copy: "Recursos para Proveedores de Capacitación",
                url: "/training-provider-resources",
                itemId: "4Rrw3ktcpB7VQU1LcOpeTS",
              },
              {
                copy: "SkillUp NJ",
                url: "https://nj.metrixlearning.com/landing.cfm",
                itemId: "4bKG1PzbEV649oyXknBUUe",
              },
            ],
          },
        },
        {
          copy: "Explorar Carreras",
          itemId: "2ZTghf6TUoZgAKd2cUHy4P",
          subItemsCollection: {
            items: [
              {
                copy: "Navegador de Carreras de NJ",
                url: "/navigator",
                itemId: "7bUSUzaugCaaJiO8q5VFP2",
              },
              {
                copy: "Rutas de Carrera de NJ",
                url: "/career-pathways",
                itemId: "4qo1suhSHJek5pbDGLYqUZ",
              },
            ],
          },
        },
        {
          copy: "Encontrar Apoyo y Asistencia",
          classes: "no-link",
          itemId: "5SPeK1gJLpMBLGuTFEAVmg",
          subItemsCollection: {
            items: [
              {
                copy: "Explorar Apoyo por Categoría",
                url: "/support-resources",
                itemId: "4ec33mgyIj1ghaZ9GYNCGR",
              },
              {
                copy: "Apoyo para Carrera",
                url: "/support-resources/career-support",
                itemId: "3YouqKxnUQiRodAUNnv8nR",
              },
              {
                copy: "Asistencia para Matrícula",
                url: "/support-resources/tuition-assistance",
                itemId: "2gqhjZAj8srnVuvuVdXUBf",
              },
              {
                copy: "Otra Asistencia",
                url: "/support-resources/other",
                itemId: "6f98Wu1WAn4oyLkrbd22y7",
              },
              {
                copy: "Recursos para Proveedores de Capacitación",
                url: "/training-provider-resources",
                itemId: "4eb9EtRlHD03bDlUfJ8mkt",
              },
              {
                copy: "Preguntas Frecuentes",
                url: "/faq",
                itemId: "4gnP8eZ8H64LIclMrHvnx0",
              },
              {
                copy: "Servicios de Carrera",
                url: "https://nj.gov/labor/career-services/",
                itemId: "25V7xd5vZEG9BLihkBR7xW",
              },
            ],
          },
        },
        {
          copy: "Contáctenos",
          classes: "no-link",
          url: "/contact",
          itemId: "5rEIg0E1yASZgJXNm537PX",
          noIndicator: true,
          subItemsCollection: {
            items: [
              {
                copy: "Contáctenos",
                noIndicator: true,
                url: "/contact",
                itemId: "2Mi5UQIdSBuhJXl1CckrWA",
              },
              {
                copy: "Prensa y Medios",
                url: "/press-and-media",
                itemId: "3p1jv1b0gk3J5jYk2t1e8",
              },
            ],
          },
        },
      ],
    },
  },
};

export const FOOTER_NAV_2_DATA = {
  title: "Footer Nav Menu #2",
  heading: "Statewide",
  itemId: "3WHbfXiLFSBXRC24QCq8H6",
  en: {
    topLevelItemsCollection: {
      items: [
        {
          copy: "Governor Phil Murphy",
          url: "https://nj.gov/governor/admin/about/",
          itemId: "4QXbYxnVsikNOewkJdCaan",
        },
        {
          copy: "Lt. Governor Tahesha Way",
          url: "https://nj.gov/governor/admin/lt/",
          itemId: "xijXVpdJC9tPN5ltS0VIc",
        },
        {
          copy: "NJ Home",
          url: "https://www.nj.gov/",
          itemId: "NySIFL5Jt6kfpVDZdwsHm",
        },
        {
          copy: "Services A to Z",
          url: "https://nj.gov/nj/gov/njgov/alphaserv.html",
          itemId: "5KqEtgQ4t2WJiV4P4LJoMd",
        },
        {
          copy: "Departments/Agencies",
          url: "https://nj.gov/nj/gov/deptserv/",
          itemId: "4CxazQO8X7UXqr21bHcdWV",
        },
        {
          copy: "FAQs",
          screenReaderOnlyCopy: "Frequently Asked Questions",
          url: "https://nj.gov/faqs/",
          itemId: "3MAVdnaYnsNzETk2KR9cZZ",
        },
        {
          copy: "Privacy Notice",
          url: "https://www.nj.gov/nj/privacy.html",
          itemId: "6rZpK2xCuC3agD2FpwTG1L",
        },
        {
          copy: "Legal Statement & Disclaimers",
          url: "https://www.nj.gov/nj/legal.html",
          itemId: "3TvAmUKAxChvps0fE5lWkZ",
        },
        {
          copy: "Accessibility Statement",
          url: "https://www.nj.gov/nj/accessibility.html",
          itemId: "7lf0OplC7PRDXZ6Ohjr7Kn",
        },
      ],
    },
  },
  es: {
    topLevelItemsCollection: {
      items: [
        {
          copy: "Gobernador Phil Murphy",
          url: "https://nj.gov/governor/admin/about/",
          itemId: "4QXbYxnVsikNOewkJdCaan",
        },
        {
          copy: "Vicegobernadora Tahesha Way",
          url: "https://nj.gov/governor/admin/lt/",
          itemId: "xijXVpdJC9tPN5ltS0VIc",
        },
        {
          copy: "Inicio NJ",
          url: "https://www.nj.gov/",
          itemId: "NySIFL5Jt6kfpVDZdwsHm",
        },
        {
          copy: "Servicios de la A a la Z",
          url: "https://nj.gov/nj/gov/njgov/alphaserv.html",
          itemId: "5KqEtgQ4t2WJiV4P4LJoMd",
        },
        {
          copy: "Departamentos/Agencias",
          url: "https://nj.gov/nj/gov/deptserv/",
          itemId: "4CxazQO8X7UXqr21bHcdWV",
        },
        {
          copy: "Preguntas Frecuentes",
          screenReaderOnlyCopy: "Preguntas Frecuentes",
          url: "https://nj.gov/faqs/",
          itemId: "3MAVdnaYnsNzETk2KR9cZZ",
        },
        {
          copy: "Aviso de Privacidad",
          url: "https://www.nj.gov/nj/privacy.html",
          itemId: "6rZpK2xCuC3agD2FpwTG1L",
        },
        {
          copy: "Declaración Legal y Descargos de Responsabilidad",
          url: "https://www.nj.gov/nj/legal.html",
          itemId: "3TvAmUKAxChvps0fE5lWkZ",
        },
        {
          copy: "Declaración de Accesibilidad",
          url: "https://www.nj.gov/nj/accessibility.html",
          itemId: "7lf0OplC7PRDXZ6Ohjr7Kn",
        },
      ],
    },
  },
};
